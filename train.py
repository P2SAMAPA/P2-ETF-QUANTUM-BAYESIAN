...
from quantum_bayes import bayesian_scores
...
def run_for_window(returns, window_days):
    ...
    try:
        raw_scores = bayesian_scores(
            ret_window,
            tau=config.TAU,
            use_petz=config.USE_PETZ
        )
    ...
    top_etfs = [{"ticker": t, "bayes_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    out_file = f"output/quantum_bayes_{timestamp}.json"
